...
#' @param color optional, named character vector, names are specifying 
...
FlexPivot = function( dataset, id, transpose, columns, 
	...
	if( length( color ) > 0 ){
		...
		color_data = matrix( "black", ncol = length( columns ), nrow = nrow(dataset) )
...
#	columns = paste0( ".", seq_len( length(columns) ), "_",columns )
...
#		headers[nrow(headers), ] = gsub("^\\.[0-9]+", "", headers[nrow(headers), ])
#	}
	...
	ft = addFooterRow( ft, value = "", colspan = ncol(body) )
	...
	ft[1,to="footer", side = "top"] = borderSolid()
	ft
}
...
table_transpose = function( dataset, id, transpose, columns, columns.transpose ){
	...
	if( length( transpose ) > 0 ){
		...
		levels(data_transpose[,1]) = paste0( ".", 
				seq_along( levels(data_transpose[,1]) ), "_", 
				levels(data_transpose[,1]) )
		data = cbind( data, data_transpose )
	}
	data = gather_(data, key_col = "column_name", value_col = "value", gather_cols = names( subset ) )
